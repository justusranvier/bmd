// Originally derived from: btcsuite/btcd/btcec/example_test.go
...
func Example_signMessage() {
	...
	privKey, pubKey := bmec.PrivKeyFromBytes(bmec.S256(), pkBytes)

	// Sign a message using the private key.
	message := "test message"
	messageHash := doubleSha256([]byte(message))
...
func Example_verifySignature() {
	...
	pubKey, err := bmec.ParsePubKey(pubKeyBytes, bmec.S256())
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	signature, err := bmec.ParseSignature(sigBytes, bmec.S256())
	if err != nil {
		fmt.Println(err)
		return
	}
...
func Example_encryptMessage() {
	// Decode the hex-encoded of the recipient.
	...
	message := "test message"
	ciphertext, err := bmec.Encrypt(pubKey, []byte(message))
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	privKey, _ := bmec.PrivKeyFromBytes(bmec.S256(), pkBytes)

	// Try decrypting and verify if it's the same message.
	plaintext, err := bmec.Decrypt(privKey, ciphertext)
	if err != nil {
		fmt.Println(err)
		return
	}
