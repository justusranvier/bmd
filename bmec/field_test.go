// Originally derived from: btcsuite/btcd/btcec/field_test.go
...
func TestSetInt(t *testing.T) {
	...
	for i, test := range tests {
		f := bmec.NewFieldVal().SetInt(test.in)
		result := f.TstRawInts()
...
func TestStringer(t *testing.T) {
	tests := []struct {
		...
		// 2^256-4294968273 (the bmec prime, so should result in 0)
		{
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		f := bmec.NewFieldVal().SetHex(test.in)
		result := f.String()
...
func TestNormalize(t *testing.T) {
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		f := bmec.NewFieldVal().TstSetRawInts(test.raw).Normalize()
		result := f.TstRawInts()
...
func TestEquals(t *testing.T) {
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		f := bmec.NewFieldVal().SetHex(test.in1).Normalize()
		f2 := bmec.NewFieldVal().SetHex(test.in2).Normalize()
		result := f.Equals(f2)
		if result != test.expected {
...
func TestNegate(t *testing.T) {
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		f := bmec.NewFieldVal().SetHex(test.in).Normalize()
		expected := bmec.NewFieldVal().SetHex(test.expected).Normalize()
