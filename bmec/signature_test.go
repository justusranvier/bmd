// Originally derived from: btcsuite/btcd/btcec/signature_test.go
...
func TestSignatures(t *testing.T) {
	for _, test := range signatureTests {
		var err error
		if test.der {
			_, err = bmec.ParseDERSignature(test.sig, bmec.S256())
		} else {
			_, err = bmec.ParseSignature(test.sig, bmec.S256())
		}
		if err != nil {
			if test.isValid {
	...
	tests := []struct {
		...
		// fda204502a3345e08afd6af27377c052e77f1fefeaeb31bdd45f1e1237ca5470
		{
			...
			&bmec.Signature{
				...
				S: new(big.Int).Add(fromHex("00c1a251bbecb14b058a8bd77f65de87e51c47e95904f4c0e9d52eddc21c1415ac"), bmec.S256().N),
			},
			[]byte{
...
func testSignCompact(t *testing.T, tag string, curve *bmec.KoblitzCurve,
	data []byte, isCompressed bool) {
	tmp, _ := bmec.NewPrivateKey(curve)
	priv := (*bmec.PrivateKey)(tmp)

	hashed := []byte("testing")
	sig, err := bmec.SignCompact(curve, priv, hashed, isCompressed)
	if err != nil {
		t.Errorf("%s: error signing: %s", tag, err)
		return
	}

	pk, wasCompressed, err := bmec.RecoverCompact(curve, sig, hashed)
	if err != nil {
...
func TestSignCompact(t *testing.T) {
	for i := 0; i < 256; i++ {
		...
		compressed := i%2 != 0
		testSignCompact(t, name, bmec.S256(), data, compressed)
	}
}
