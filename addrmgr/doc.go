// Originally derived from: btcsuite/btcd/addrmgr/doc.go
...
In order maintain the peer-to-peer Bitmessage network, there needs to be a
source of addresses to connect to as nodes come and go. The Bitmessage protocol
provides addr messages to allow peers to communicate known addresses with each
other. However, there needs to a mechanism to store those results and select
peers from them. It is also important to note that remote peers can't be
trusted to send valid peers nor attempt to provide you with only peers they
